...
<body style="margin: 40px;">
    ...
    <% if (sessionUid != null) { 		// 로그인 된 상태 %>	
    	<button onclick="location.href='/ch12/users2/logout'">로그아웃</button>
    	<%= session.getAttribute("uname") %>님 환영합니다.
    <% } else { %>
	    <button onclick="location.href='/ch12/users2/login.html'">로그인</button>
	<% } %>
    <hr>
    <table border="1">
        <tr>
        ...
        <tr>
            ...
            <% } else { %>
                <button onclick="location.href='/ch12/users2/update?uid=<%=u.getUid()%>'">수정</button>
            <% } %>
            ...
            <% } else { %>
                <button onclick="location.href='/ch12/users2/delete?uid=<%=u.getUid()%>'">삭제</button>
            <% } %>
            </td>
        </tr>
        <% } %>
    ...
    <br>
    <a href="/ch12/users2/register.jsp">회원 가입</a>
</body>
</html>